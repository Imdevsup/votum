  ...
  await app.register(cors, {
    origin: (origin, cb) => {
      ...
      try {
        ...
        // Content scripts on github.com forward their host as the request
        // origin, so the API has to whitelist GitHub explicitly. Subdomains
        // (gist.github.com, etc.) are intentionally excluded — the
        // extension's manifest only injects on github.com itself.
        if (host === 'github.com') return cb(null, true);
        // Permit any *.vercel.app preview/production domains so we don't
        // have to redeploy the backend every time the web frontend gets a
        // new preview URL.
